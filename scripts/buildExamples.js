...
const exampleProjects = [
    { entry: "src/index_2d-only.js", output: "example_2d-only" },
    { entry: "src/index_3d-only.js", output: "example_3d-only" }
];

for (let i = 0; i < exampleProjects.length; ++i) {
    let project = exampleProjects[i];
    config.entry.index = [path.join(BASE_DIR, project.entry)];
    config.output.path = path.join(BASE_DIR, project.output);
